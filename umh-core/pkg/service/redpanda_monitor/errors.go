...
var (
	...
	ErrServiceConnectionRefused = errors.New("connection refused, while attempting to fetch metrics from redpanda. This is expected during the startup phase of the redpanda service, when the service is not yet ready to receive connections")

	// ErrServiceConnectionTimedOut indicates the service connection timed out
	ErrServiceConnectionTimedOut = errors.New("connection timed out, while attempting to fetch metrics from redpanda. This can happen if the redpanda service or the system is experiencing high load")
