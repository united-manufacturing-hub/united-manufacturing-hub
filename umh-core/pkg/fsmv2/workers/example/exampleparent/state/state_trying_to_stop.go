...
func (s *TryingToStopState) Next(snapAny any) fsmv2.NextResult[any, any] {
	...
	if snap.Observed.ID == "" {
		return fsmv2.Result[any, any](s, fsmv2.SignalNone, &action.StopAction{}, "ID not set, executing StopAction", nil)
	}

	// All children must be stopped before transitioning.
	if snap.Observed.ChildrenHealthy == 0 && snap.Observed.ChildrenUnhealthy == 0 {
		return fsmv2.Result[any, any](&StoppedState{}, fsmv2.SignalNone, nil, "All children stopped, transitioning to Stopped", nil)
	}

	return fsmv2.Result[any, any](s, fsmv2.SignalNone, nil, "Gracefully stopping all children", nil)
}
