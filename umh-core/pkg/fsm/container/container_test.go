...
var _ = Describe("Container FSM", func() {
	...
	Context("When newly created", func() {
		It("Should initially be in lifecycle state `to_be_created` -> then `creating` -> `monitoring_stopped`", func() {
			...
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 1}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
			Expect(inst.GetCurrentFSMState()).To(Equal("creating"))

			// next reconcile
			err, did = inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 2}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
	...
	Context("Lifecycle transitions", func() {
		BeforeEach(func() {
			...
			inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 10}, mockFS)
			inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 11}, mockFS)
			Expect(inst.GetCurrentFSMState()).To(Equal(container.OperationalStateStopped))
		})
		...
		It("Should go from `monitoring_stopped` to `degraded` on start", func() {
			...
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 12}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
			Expect(inst.GetCurrentFSMState()).To(Equal(container.OperationalStateStarting))

			// next reconcile
			err, did = inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 13}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
		...
		It("Should remain `monitoring_stopped` if desired is `stopped`", func() {
			...
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 14}, mockFS)
			Expect(err).ToNot(HaveOccurred())
	...
	Context("When monitoring is running", func() {
		BeforeEach(func() {
			...
			inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 20}, mockFS)
			inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 21}, mockFS)
			// set desired = active
			inst.SetDesiredFSMState(container.OperationalStateActive)
			// cause start
			inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 22}, mockFS)
			Expect(inst.GetCurrentFSMState()).To(Equal(container.OperationalStateStarting))

			// next reconcile
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 23}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
			Expect(inst.GetCurrentFSMState()).To(Equal(container.OperationalStateDegraded))
		})
		...
		It("Transitions from degraded -> active if metrics healthy", func() {
			...
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 24}, mockFS)
			Expect(err).ToNot(HaveOccurred())
			Expect(did).To(BeTrue())
		...
		It("Remains degraded if metrics are not healthy", func() {
			...
			err, did := inst.Reconcile(ctx, snapshot.SystemSnapshot{Tick: 25}, mockFS)
			Expect(err).ToNot(HaveOccurred())
