...
High CPU usage alone is not a problem: an instance can run at high utilization and stay healthy as long as there is spare headroom, meaning a free core on the host or room under the limit. *Busy* is not the same as *starved* or *full*. UMH degrades when the reserve is used up (less than one host core free, or under 10% of the limit) or when work is actually being delayed; where the system reports them, it shows the three sharp signals: throttling, pressure, and steal. These are kernel-defined facts, not interpretations. The minimum recommendation is 4 vCPU; below that the 1-core headroom reserve leaves too little room for work. (For the reasoning, see our write-up on [why average CPU utilization is the wrong signal](https://www.theocharis.dev/blog/why-we-should-get-rid-of-average-cpu-utilization/).)
