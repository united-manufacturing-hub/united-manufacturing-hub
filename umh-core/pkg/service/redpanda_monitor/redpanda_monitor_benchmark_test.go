...
func prepareSampleLogEntries() []process_shared.LogEntry {
	...
	entries = append(entries, process_shared.LogEntry{Content: metricsGZIPHex})
	...
	entries = append(entries, process_shared.LogEntry{Content: clusterConfigGZIPHex})

	// Add CLUSTERCONFIG_END_MARKER
	entries = append(entries, process_shared.LogEntry{Content: CLUSTERCONFIG_END_MARKER})
