...
func (s *StartingState) Next(snapAny any) fsmv2.NextResult[any, any] {
	...
	// Catch-all: remain in Starting while waiting for authentication
	return fsmv2.Result[any, any](s, fsmv2.SignalNone, nil,
		fmt.Sprintf("starting: hasValidToken=%t, consecutiveErrors=%d",
			snap.Observed.HasValidToken(), snap.Observed.ConsecutiveErrors))
}
