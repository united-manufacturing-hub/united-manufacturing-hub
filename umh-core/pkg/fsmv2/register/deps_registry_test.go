...
var _ = Describe("register deps registry", func() {
	...
	Context("concurrent Set/Get", func() {
		It("is race-free under concurrent goroutines on disjoint keys", func() {
			const goroutines = 50
		...
		It("is race-free with many goroutines hammering a single shared key", func() {
			// Disjoint-key concurrency only exercises map-bucket protection. A
			// real-world race against `depsRegistry["transport"]` looks like
			// many readers and a handful of writers contending for one entry.
			// `go test -race` only catches that race if writers and readers
			// touch the same key.
			const key = "shared-race-key"
			const writers = 8
			const readers = 32
	...
	Context("factory-closure integration", func() {
		BeforeEach(func() {
