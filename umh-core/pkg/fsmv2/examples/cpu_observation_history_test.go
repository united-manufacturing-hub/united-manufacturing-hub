...
func applyCPUChange(into *cpuObservation, changes *storage.Diff) {
	set := func(field string, value interface{}) {
		...
		case "signalsCapable":
			into.SignalsCapable = asInt(value)
		case "degraded":
...
// asInt reads a count back out of a delta. The document a delta is computed
// from has been through JSON, so a count arrives as a float64 rather than as
// the int the worker published; the int arm is here because nothing in the
// store's contract promises it stays that way.
