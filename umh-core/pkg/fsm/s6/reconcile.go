...
func (s *S6Instance) reconcileOperationalStates(ctx context.Context, services serviceregistry.Provider, currentState string, desiredState string, currentTime time.Time) (err error, reconciled bool) {
	...
	// Operational HealthBad escape is handled by the top-level Health() dispatch
	// in reconcileStateTransition. The remaining HealthBad branches below
	// (skip-stop, consider-stopped) are defensive belt-and-suspenders — they
	// rarely fire now that CheckArtifactsHealth no longer reports HealthBad
	// for s6's autogenerated supervise dirs. Slated for a separate cleanup
	// pass once the recovery directive bakes one release.
