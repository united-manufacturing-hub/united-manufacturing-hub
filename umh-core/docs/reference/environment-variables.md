...
| USE\_CANONICALIZE\_FAST | 0, 1 (default: 1) | Compares a bridge's desired and running configuration by walking the values directly. Set to 0 and restart the container to fall back to the slower YAML round-trip, the reference implementation, if a configuration is ever reported as changed on every tick. Any other value also falls back, and logs a warning naming it |
