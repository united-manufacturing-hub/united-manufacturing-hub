...
func (s *S6Instance) Reconcile(ctx context.Context, currentSnapshot snapshot.SystemSnapshot, filesystemService filesystem.Service) (err error, reconciled bool) {
	start := time.Now()
	...
	if s.baseFSMInstance.ShouldSkipReconcileBecauseOfError(currentSnapshot.Tick) {
		err := s.baseFSMInstance.GetBackoffError(currentSnapshot.Tick)
	...
	if err := s.reconcileExternalChanges(ctx, filesystemService, currentSnapshot.Tick, start); err != nil {
		// If the service is not running, we don't want to return an error here, because we want to continue reconciling
		if !errors.Is(err, s6service.ErrServiceNotExist) {
			s.baseFSMInstance.SetError(err, currentSnapshot.Tick)
