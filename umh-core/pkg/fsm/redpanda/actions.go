...
func (r *RedpandaInstance) IsRedpandaDegraded(currentTime time.Time, logWindow time.Duration) (bool, string) {
	...
	runningWithoutErrors, reasonRunningWithoutErrors := r.IsRedpandaRunningWithoutErrors(currentTime, logWindow)
	...
	if !runningWithoutErrors {
		return true, reasonRunningWithoutErrors
	}
	return false, ""
}
