...
// ProcessMessages processes a EndOrder kafka message, by creating an database connection, decoding the json payload, retrieving the required additional database id's (like AssetTableID or ProductTableID) and then inserting it into the database and committing
func (c EndOrder) ProcessMessages(msg internal.ParsedMessage) (putback bool, err error, forcePbTopic bool) {
