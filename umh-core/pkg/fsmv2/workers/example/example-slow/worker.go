...
import (
	...
	"github.com/united-manufacturing-hub/united-manufacturing-hub/umh-core/pkg/fsmv2/workers/example/example-slow/snapshot"
	"github.com/united-manufacturing-hub/united-manufacturing-hub/umh-core/pkg/fsmv2/workers/example/example-slow/state"
)

type SlowWorker struct {
	*helpers.BaseWorker[*SlowDependencies]
...
func (w *SlowWorker) CollectObservedState(ctx context.Context) (fsmv2.ObservedState, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
...
func (w *SlowWorker) DeriveDesiredState(spec interface{}) (fsmv2types.DesiredState, error) {
	if spec == nil {
...
func init() {
	_ = factory.RegisterSupervisorFactory[snapshot.SlowObservedState, *snapshot.SlowDesiredState](
