...
func (f *FreshnessChecker) extractTimestamp(snapshot *fsmv2.Snapshot) (time.Time, bool) {
	...
	if !ok {
		f.logger.Warn("observed_state_type_unknown",
	...
	if !exists {
		f.logger.Warn("observed_state_missing_timestamp",
			deps.HierarchyPath(snapshot.Identity.HierarchyPath),
			deps.String("action", "assuming_fresh"))
	...
	case string:
		...
		if err != nil {
			f.logger.Warn("observed_state_invalid_timestamp",
				deps.HierarchyPath(snapshot.Identity.HierarchyPath),
				deps.String("value", v),
				deps.String("action", "assuming_fresh"))
	...
	default:
		f.logger.Warn("observed_state_unknown_timestamp_type",
			deps.HierarchyPath(snapshot.Identity.HierarchyPath),
...
func (f *FreshnessChecker) IsTimeout(snapshot *fsmv2.Snapshot) bool {
	...
	if isTimedOut {
		f.logger.Warn("observed_state_timeout",
			deps.HierarchyPath(snapshot.Identity.HierarchyPath),
			deps.Duration("age", age),
