...
var _ = Describe("S6Instance FSM", func() {
	...
	Context("Creation Flow", func() {
		...
		It("should transition from to_be_created to stopped (normal creation)", func() {
			...
			currentSnapshot := snapshot.SystemSnapshot{Tick: tick}
			tick, err := fsmtest.TestS6StateTransition(ctx, currentSnapshot, instance, mockFS,
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			_, err = fsmtest.TestS6StateTransition(ctx, currentSnapshot, instance, mockFS,
				internal_fsm.LifecycleStateCreating,
		...
		It("should handle creation failure and retry", func() {
			...
			currentSnapshot := snapshot.SystemSnapshot{Tick: tick}
			tick, err := fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, internal_fsm.LifecycleStateToBeCreated, 3)
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Starting Flow", func() {
		...
		It("should remain in starting until service is actually up", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStarting, 3)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should handle start failure and retry after backoff", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStopped, 3)
			Expect(err).NotTo(HaveOccurred())
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			fsmtest.ResetInstanceError(instance, currentSnapshot, mockFS)
	...
	Context("Stopping Flow", func() {
		...
		It("should remain in stopping until service is actually down", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStopping, 3)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should handle stop failure and retry after backoff", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateRunning, 3)
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Error & Backoff Handling", func() {
		...
		It("should apply backoff when operations fail repeatedly", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStopped, 100)
		...
		It("should reset backoff after a successful operation", func() {
			...
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStopped, 5)
			Expect(err).NotTo(HaveOccurred())
			Expect(mockService.StartCalled).To(BeTrue())
			Expect(instance.GetError()).NotTo(BeNil())
			...
			mockService.StartCalled = false
			currentSnapshot = snapshot.SystemSnapshot{Tick: tick}
			tick, err = fsmtest.VerifyStableState(ctx, currentSnapshot, instance, mockFS, s6fsm.OperationalStateStopped, 2)
			Expect(err).NotTo(HaveOccurred())
