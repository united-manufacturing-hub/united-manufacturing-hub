...
func (p *ProtocolConverterInstance) Reconcile(ctx context.Context, snapshot fsm.SystemSnapshot, services serviceregistry.Provider) (error, bool) {
	start := time.Now()
	...
	if p.baseFSMInstance.IsRemoving() {
		...
		err = p.reconcileExternalChanges(ctx, services, snapshot)
		if err != nil {
...
func (p *ProtocolConverterInstance) reconcileStateTransition(ctx context.Context, services serviceregistry.Provider, currentTime time.Time) (error, bool) {
	start := time.Now()
...
func (p *ProtocolConverterInstance) reconcileOperationalStates(ctx context.Context, services serviceregistry.Provider, currentState string, desiredState string, currentTime time.Time) (error, bool) {
	start := time.Now()
...
func (p *ProtocolConverterInstance) reconcileTransitionToActive(ctx context.Context, services serviceregistry.Provider, currentState string, currentTime time.Time) (err error, reconciled bool) {
	...
	if currentState == OperationalStateStopped {
		...
		err := p.StartInstance(ctx, services.GetFileSystem())
		if err != nil {
			return err, false
		}
...
func (p *ProtocolConverterInstance) reconcileTransitionToStopped(ctx context.Context, services serviceregistry.Provider, currentState string) (err error, reconciled bool) {
	...
	case OperationalStateStopping:
		...
		return nil, false
	default:
		err := p.StopInstance(ctx, services.GetFileSystem())
		if err != nil {
			return err, false
		}
