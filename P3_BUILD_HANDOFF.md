...
- [ ] **R5 — Sentry-once at deadline (`F16`)** · in flight (superseded by the completed entry below).
  - `FSMLogger.SentryWarn` naming never-measured signals, once per worker, never per tick, never via a Poll error; no-PSI boxes fire nothing. A never-measured signal is not actionable, so `SentryError` would page on-call for noise.
- [x] **R5 — Sentry-once at deadline (`F16`)** · tdd-commit + conductor closure + independent review · commit `73053ca43` · 18/18 green · **reviewed GO**.
  - `FSMLogger.SentryWarn` naming never-measured capable signals, ONCE per worker (monotonic latch, never per tick), never via a Poll error, never on a no-PSI box. A warning, not an error: a signal that was never measured is not actionable, so raising it as an error would page on-call for noise.
  - The event name is the fixed snake_case literal `cpu_admission_deadline_never_measured_signal`, and every dynamic value (signal names, measured/capable shortfall, window) rides in structured `deps.Field`s. The event name has to be fixed because both the Sentry fingerprint and the debouncer are built from the zap log MESSAGE (`sentry/hook.go:126`, `:129`; `BuildFingerprint` at `:322-336`) — an interpolated message would give every distinct signal combination its own issue and defeat the debouncer. The fields are NOT searchable: `hook.go` promotes only a fixed tag set, and everything else lands in `Contexts["umh_context"]`, which `sentry/doc.go` describes as visible on the issue page but not searchable. Grouping comes from the fixed event name; the fields are what an operator reads inside that one issue.
  - Two guards added on top (2026-08-08): the past-deadline verdict and message are pinned byte-for-byte, and the `SentryWarn` routing arguments (`feature`, `hierarchy_path`) are asserted — previously the event name and payload were pinned while the two routing arguments went unchecked.
