...
VALUES_FILE= ./deployment/united-manufacturing-hub/values.yaml
...
CTR_TAG=latest
## Container image
CTR_IMG=barcodereader factoryinsight grafana-umh hivemq-init kafka-bridge kafka-init kafka-to-postgresql kafka-to-postgresql-v2 metrics mqtt-kafka-bridge sensorconnect
## Space-separated list of workloads. Each workload is a string in the form <type>:<name>
WORKLOADS=sts:kafka svc:kafka deploy:factoryinsight-deployment deploy:opcuasimulator-deployment deploy:iotsensorsmqtt sts:hivemqce sts:nodered sts:sensorconnect
...
helm-test-local-containers: CHART=./deployment/united-manufacturing-hub
helm-test-local-containers: VALUES_FILE = ./deployment/united-manufacturing-hub/values.yaml
helm-test-local-containers: VALUES= barcodereader.image.repository=$(CTR_REPO)/barcodereader barcodereader.image.tag=$(CTR_TAG) sensorconnect.image=$(CTR_REPO)/sensorconnect sensorconnect.tag=$(CTR_TAG) kafkabridge.image.repository=$(CTR_REPO)/kafkabridge kafkabridge.image.tag=$(CTR_TAG) kafkabridge.initContainer.repository=$(CTR_REPO)/kafka-init kafkabridge.initContainer.tag=$(CTR_TAG) factoryinsight.image.repository=$(CTR_REPO)/factoryinsight factoryinsight.image.tag=$(CTR_TAG) kafkatopostgresql.image.repository=$(CTR_REPO)/kafka-to-postgresql kafkatopostgresql.image.tag=$(CTR_TAG) kafkatopostgresql.initContainer.repository=$(CTR_REPO)/kafka-init kafkatopostgresql.initContainer.tag=$(CTR_TAG) kafkatopostgresqlv2.image.repository=$(CTR_REPO)/kafka-to-postgresql-v2 kafkatopostgresqlv2.image.tag=$(CTR_TAG) kafkatopostgresqlv2.initContainer.repository=$(CTR_REPO)/kafka-init kafkatopostgresqlv2.initContainer.tag=$(CTR_TAG) tulipconnector.image.repository=$(CTR_REPO)/tulip-connector tulipconnector.image.tag=$(CTR_TAG) mqttkafkabridge.image.repository=$(CTR_REPO)/mqtt-kafka-bridge mqttkafkabridge.image.tag=$(CTR_TAG) mqttkafkabridge.initContainer.repository=$(CTR_REPO)/kafka-init mqttkafkabridge.initContainer.tag=$(CTR_TAG) metrics.image.repository=$(CTR_REPO)/metrics metrics.image.tag=$(CTR_TAG)
