...
type CompactDeltasAction struct {
	RetentionWindow time.Duration
}

func NewCompactDeltasAction(retentionWindow time.Duration) *CompactDeltasAction {
	return &CompactDeltasAction{RetentionWindow: retentionWindow}
}
...
func (a *CompactDeltasAction) Execute(ctx context.Context, depsAny any) error {
	...
	d.MetricsRecorder().IncrementCounter(deps.CounterCompactionCyclesTotal, 1)
	d.MetricsRecorder().SetGauge(deps.GaugeLastCompactionDurationMs, float64(duration.Milliseconds()))

	return nil
}

func (a *CompactDeltasAction) Name() string { return "CompactDeltas" }
