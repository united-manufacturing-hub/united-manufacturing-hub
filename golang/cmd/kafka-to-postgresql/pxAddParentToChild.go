...
func (c AddParentToChild) ProcessMessages(msg kafka2.ParsedMessage) (putback bool, err error) {
	...
	if !success {
		return true, fmt.Errorf("failed to get UniqueProductID for ChildAID: %s, AssetTableID: %d", *sC.ChildAID, AssetTableID)
	}

	var ParentUID uint32
	ParentUID, success = GetLatestParentUniqueProductID(*sC.ParentAID, AssetTableID)
	if !success {
		return true, fmt.Errorf("failed to get LatestParentUniqueProductID for ParentAID: %s, AssetTableID: %d", *sC.ParentAID, AssetTableID)
	}
