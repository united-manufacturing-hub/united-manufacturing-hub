...
var _ = Describe("CPU blind ScenarioV2", func() {
	...
	It("reports a machine it cannot measure as healthy, and a cgroup it cannot read as a poll error", func() {
		...
		// And the count of signals this box can still answer does not move.
		// Both of them are still CAPABLE — something on this box could answer
		// each — even though one of the two has just lost the only file it
		// reads. Capability is not readability, and this is the reading where
		// the difference stops being academic.
		Expect(history[blind].SignalsCapable).To(Equal(2))
