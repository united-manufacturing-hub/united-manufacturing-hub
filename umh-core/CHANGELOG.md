...
- FSMv2 examplefailing worker migrated to `register.Worker[ExamplefailingConfig, ExamplefailingStatus, *FailingDependencies]` with the same WorkerBase shape refactor: custom `ExamplefailingObservedState` / `ExamplefailingDesiredState` types replaced by `fsmv2.Observation[ExamplefailingStatus]` + `*fsmv2.WrappedDesiredState[ExamplefailingConfig]`, and state files now consume `WorkerSnapshot` via `ConvertWorkerSnapshot`. The user-spec fields (`ShouldFail`, `MaxFailures`, `FailureCycles`, `RestartAfterFailures`, `RecoveryDelayMs`, `RecoveryDelayObservations`) are now carried on the config struct and synced into the runtime dependencies via a post-parse hook. `ConnectedState` now reads `TimeInCurrentStateMs` from the typed snapshot's `FrameworkMetrics` (introduced in PR3 C3) instead of the legacy embedded metrics field. Docstring/`FOLDER_WORKER_TYPE_MISMATCH` sweeps across `factory/README.md`, `supervisor/supervisor.go`, and `internal/validator/registry.go` shifted the canonical example from `Examplefailing*` → `Examplepanic*` (the last legacy-factory example worker). Internal framework change only; no user-facing API. PR3 C4.
- FSMv2 examplepanic worker deleted in its entirety (`pkg/fsmv2/workers/example/examplepanic/` directory + its blank imports in `examples/runner.go` and `architecture_test.go` + the `PanicScenario` definition in `examples/panic.go` and its registry entry + the `integration/panic_scenario_test.go` integration spec). The worker was a legacy-shape example (`factory.RegisterWorkerType` caller) superseded by the migrated examples from PR3 C1-C4; panic recovery semantics are exercised by the supervisor's own panic-recovery unit tests. Completes the example-worker sweep: zero legacy `factory.RegisterWorkerType` callers remain in `workers/**` (the `workers/example/doc.go:69` godoc code sample is prose and will be re-anchored in PR3 C9). Internal framework change only; no user-facing API. PR3 C5.
- FSMv2 `fsmv2.WrapAction[TDeps]` framework compat seam removed (`pkg/fsmv2/wrap_action.go` + `pkg/fsmv2/wrap_action_test.go` deleted). Workers pass typed actions (`&MyAction{}`) directly to `fsmv2.Transition`; the framework auto-wraps them into `Action[any]` via reflection. The deprecated block in `pkg/fsmv2/README.md` / `pkg/fsmv2/CLAUDE.md` was removed (migration is complete). The `internal/validator/transition_alias.go` matcher still flags any accidental `fsmv2.WrapAction[...]` call (defensive guardrail; the symbol no longer compiles). Internal framework change only; no user-facing API. PR3 C6.
- FSMv2 `factory.RegisterWorkerType[TObserved, TDesired]` generic registration wrapper removed from `pkg/fsmv2/factory/worker_factory.go`. All in-tree workers now register through `register.Worker[TConfig, TStatus, TDeps]` (C1-C5 migration). The runtime-visible validator error strings `FOLDER_WORKER_TYPE_MISMATCH.CorrectCode`, `REGISTRY_MISMATCH.CorrectCode`, and `REGISTRY_MISMATCH.ReferenceFile` (`pkg/fsmv2/internal/validator/registry.go`) were updated in the same commit to point operators at the `register.Worker` API. The underlying building block `factory.RegisterWorkerAndSupervisorFactoryByType` remains intact until PR3 C8. Internal framework change only; no user-facing API. PR3 C7.
- FSMv2 `factory.RegisterWorkerAndSupervisorFactoryByType` wrapper removed and the `map[string]any` "extraDeps" parameter dropped from the worker-factory closure type across the factory package (`registry` map, `RegisterFactoryByType`, `RegisterFactory`, `NewWorkerByType`, `GetFactory`). The factory closure is now 3-arg `(deps.Identity, deps.FSMLogger, deps.StateReader) fsmv2.Worker`. `register.Worker` inlines the paired `RegisterFactoryByType` + `RegisterSupervisorFactoryByType` calls directly; both panic on error, which terminates the process before any partial registration becomes observable, so the old wrapper's rollback is no longer required. Supervisor call sites in `supervisor/lifecycle.go` and `supervisor/reconciliation.go` now call `factory.NewWorkerByType` with 4 args (the 5th `map[string]any` argument is dropped; the dead-but-computed `s.deps` / `mergedDeps` fields are left for a post-PR3 YAML-schema audit). The `### RegisterWorkerAndSupervisorFactoryByType` section in `factory/README.md` is deleted (it cited both the now-removed wrapper and the C5-deleted `ExamplepanicObservedState` example). Internal framework change only; no user-facing API. PR3 C8.
- FSMv2 transport-migration residue cleaned up after PR3 C8: `cmd/main.go:603` switched from the deleted `transportSnapshot.TransportObservedState` to the current WorkerBase shape `fsmv2.Observation[transportWorker.TransportStatus]` (straight swap -- no type alias), and `pkg/fsmv2/examples/runner.go` regained the three missing blank imports for `workers/transport/state`, `workers/transport/push/state`, and `workers/transport/pull/state`. Without those imports, each state package's `init() { fsmv2.RegisterInitialState(...) }` never ran, so `GetInitialState("transport")` panicked on every supervisor tick and the transport scenario tests saw `AuthCallCount=0` because the worker was being reconstructed rather than progressing through `Stopped → Starting → AuthenticateAction`. Fix restores the transport scenario green (all Transport + Communicator specs pass); the ~20-25s per-test shutdown time is the expected 4-level cascading graceful-shutdown budget documented in `pkg/fsmv2/CLAUDE.md`, not a hang. Internal framework change only; no user-facing API. PR3 C8.5.
- FSMv2 documentation sweep after PR3 C5-C8: godoc examples (`examples/simple.go`, `fsmv2/doc.go`, `supervisor/supervisor.go`, `workers/example/doc.go`), READMEs (`factory/README.md`, `workers/application/README.md`, `workers/example/helloworld/README.md`), validator-registry error strings (`internal/validator/registry.go` FOLDER_WORKER_TYPE_MISMATCH), `pkg/fsmv2/CLAUDE.md` shutdown-cascade table, and `pkg/fsmv2/MIGRATION.md` re-anchored around the current `register.Worker[TConfig, TStatus, TDeps]` + WorkerBase API. All references to the deleted `factory.RegisterWorkerType`, `WrapAction`, `extraDeps`, and `examplepanic` symbols removed; `workers/example/helloworld/README.md` rewritten as the canonical mini-worker reference (skeleton, file-by-file guide, step-by-step feature adoption). Documentation-only change; no user-facing API. PR3 C9.
