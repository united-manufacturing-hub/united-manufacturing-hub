...
var _ = Describe("CPUHealthScenario", func() {
	It("registers the cpuhealth scenario in the registry for CLI access", func() {
		...
		Expect(scenario.YAMLConfig).To(BeEmpty(),
			"the scenario must build the worker directly with a mock-backed sampler, not a YAML-spawned one")
	})

	It("drives the worker over a mocked filesystem and shuts down clean", func() {
		...
		Expect(result.ShutdownClean).To(BeTrue(),
			"the mocked-filesystem scenario has no supervisor, so a clean shutdown is the only honest value")
