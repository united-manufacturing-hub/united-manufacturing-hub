...
var _ = Describe("CPU latch ScenarioV2", func() {
	...
	It("holds the degraded verdict through a reading under the fire mark and releases it under the clear mark", func() {
		...
		// Two signals can answer this box: pressure, and whether the machine is
		// full. There is no CPU limit, so throttling and the container's own
		// budget are not in the table, and the machine is bare metal, so steal
		// has no instrument.
		Expect(observed.Status.SignalsCapable).To(Equal(2))
	})
})
