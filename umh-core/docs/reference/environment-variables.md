...
| USE\_CANONICALIZE\_FAST | true, false (default: true) | Compares a bridge's desired and running configuration by walking the values directly. Set to false to fall back to the slower YAML round-trip, which is the reference implementation, if a configuration is ever reported as changed on every tick |
