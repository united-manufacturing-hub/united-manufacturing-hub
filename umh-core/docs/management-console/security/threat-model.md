...
| The Management Console and Auth0 | OAuth 2.0. Auth0 stores credentials and runs multi-factor challenges |
...
| --- | --- | --- |
| Outside attacker | Credential stuffing, phishing, brute force | **Protected.** Auth0 rate-limits, detects anomalies and can enforce multi-factor |
...
| Whoever holds the Account Owner, where a company has one | Taking over the company | **Accepted risk.** The account cannot be demoted, which is what makes it a recovery path. See [Fallback Access](../users-and-permissions/fallback-access.md) |

## Attack scenarios

| Attack | What stops it |
| --- | --- |
| Guessing or replaying credentials | Auth0 rate-limits sign-in and detects anomalies. Enterprise customers can require multi-factor for everyone |
...
| Reaching another company's data | A sign-in belongs to one company at a time, and a company linked to an Auth0 organization can only be joined through that organization |
...
- Auth0's own infrastructure, which Auth0 is responsible for. See [Shared Responsibility](shared-responsibility.md).
