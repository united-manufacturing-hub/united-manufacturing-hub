...
// FetchCertForEmail fetches and caches the certificate for a single email on demand.
func (h *CertHandler) FetchCertForEmail(ctx context.Context, email string) error {
	return h.fetchAndStore(ctx, email)
}
