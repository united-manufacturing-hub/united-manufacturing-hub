...
var _ = Describe("S6Manager", func() {
	...
	Context("Initialization", func() {
		It("should handle empty config without errors", func() {
			...
			err, _ := manager.Reconcile(ctx, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: emptyConfig}}, Tick: tick}, mockSvcRegistry)
			tick++
		...
		It("should initialize service in stopped state and maintain it", func() {
			...
			var nextTick uint64
			nextTick, err = fsmtest.WaitForMockedManagerInstanceState(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: configWithStoppedService}}, Tick: tick}, mockSvcRegistry,
		...
		It("should initialize service in running state when config requests it", func() {
			...
			var nextTick uint64
			nextTick, err = fsmtest.WaitForMockedManagerInstanceState(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: configWithRunningService}}, Tick: tick}, mockSvcRegistry,
	...
	Context("State Transitions", func() {
		It("should change instance state from stopped to running when config changes", func() {
			...
			nextTick, err = fsmtest.WaitForMockedManagerInstanceState(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: stoppedConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, s6fsm.OperationalStateStopped, 20)
			tick = nextTick
	...
	Context("Lifecycle", func() {
		It("should handle full lifecycle from creation to running to removal", func() {
			...
			var err error
			var nextTick uint64
			nextTick, err = fsmtest.WaitForManagerInstanceCreation(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: runningConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, 20)
			tick = nextTick
			Expect(err).NotTo(HaveOccurred(), "Failed to create instance")

			// Wait for service to reach running state
			nextTick, err = fsmtest.WaitForMockedManagerInstanceState(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: runningConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, s6fsm.OperationalStateRunning, 20)
			tick = nextTick
			...
			nextTick, err = fsmtest.WaitForManagerInstanceRemoval(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: emptyConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, 25)
			tick = nextTick
	...
	Context("Updates", func() {
		...
		It("should stop and remove an instance when a service is removed from config", func() {
			...
			nextTick, err = fsmtest.WaitForManagerInstanceRemoval(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: updatedConfig}}, Tick: tick}, mockSvcRegistry,
				service2Name, 15)
			tick = nextTick
	...
	Context("Error Handling", func() {
		It("should recover instance after transient errors", func() {
			...
			var err error
			var nextTick uint64
			nextTick, err = fsmtest.WaitForManagerInstanceCreation(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: serviceConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, 20)
			tick = nextTick
			Expect(err).NotTo(HaveOccurred(), "Failed to create instance")

			nextTick, err = fsmtest.WaitForMockedManagerInstanceState(ctx, manager, fsm.SystemSnapshot{CurrentConfig: config.FullConfig{Internal: config.InternalConfig{Services: serviceConfig}}, Tick: tick}, mockSvcRegistry,
				serviceName, s6fsm.OperationalStateStopped, 20)
			tick = nextTick
			Expect(err).NotTo(HaveOccurred(), "Failed to reach stopped state")
