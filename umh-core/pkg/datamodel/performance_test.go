...
var _ = Describe("Performance", func() {
	...
	It("should have reasonable memory usage", func() {
		...
		// Handle potential underflow if GC freed more memory than we allocated
		var memUsed uint64
		if memAfter.Alloc >= memBefore.Alloc {
			memUsed = memAfter.Alloc - memBefore.Alloc
		} else {
			memUsed = 0 // GC freed more memory than we used
		}

		GinkgoWriter.Printf("Memory used for 1000 validations: %d bytes (%.2f KB)\n", memUsed, float64(memUsed)/1024)

		// Verify reasonable memory usage (less than 1MB for 1000 validations)
		// Since the validator is highly optimized, it should use minimal memory
		Expect(memUsed).To(BeNumerically("<=", 1024*1024),
			"Memory usage too high: %d bytes", memUsed)
