...
func ConfigureServiceForState(mockService *s6_default.MockService, servicePath string, state string) {
...
func ConfigureS6ServiceConfig(mockService *s6_default.MockService) {
...
func ConfigureS6State(instance *s6fsm.S6Instance, targetState string) error {
	mockService, ok := instance.GetService().(*s6_default.MockService)
	if !ok {
...
func SetupS6Instance(
	...
	name string,
	desiredState string,
) (*s6fsm.S6Instance, *s6_default.MockService, string) {
