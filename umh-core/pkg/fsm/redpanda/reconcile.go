...
func (b *RedpandaInstance) Reconcile(ctx context.Context, tick uint64) (err error, reconciled bool) {
	start := time.Now()
	...
	if b.baseFSMInstance.ShouldSkipReconcileBecauseOfError(tick) {
		...
		if backoff.IsPermanentFailureError(err) {
			...
			if b.IsRemoved() || b.IsRemoving() || b.IsStopping() || b.IsStopped() {
				...
				b.service.ForceRemoveRedpanda(ctx)
				return err, false
			} else {
...
func (b *RedpandaInstance) reconcileExternalChanges(ctx context.Context, tick uint64) error {
	start := time.Now()
	defer func() {
...
func (b *RedpandaInstance) reconcileStateTransition(ctx context.Context, currentTime time.Time) (err error, reconciled bool) {
	...
	b.baseFSMInstance.GetLogger().Infof("currentState: %s, desiredState: %s", currentState, desiredState)
...
func (b *RedpandaInstance) reconcileStartingState(ctx context.Context, currentState string, currentTime time.Time) (err error, reconciled bool) {
	...
	case OperationalStateStartingConfigLoading:
		...
		// Now check whether redpanda has loaded the config
		if !b.IsRedpandaConfigLoaded() {
			return nil, false
		}
