...
- [x] **Rung 3 — byte-identical output invariant (F16/F17 guard)** · subagent + review · commit `1a137bc76` · **76/76 cpuhealth + 106/106 diagnosis**.
  - What it pins: a healthy no-PSI dead-zone box renders the SAME customer-visible output — verdict healthy, no causes, `PsiApplies` false, `LimitedVisibility` true, message carries the limited-visibility advisory — through the REAL `DeriveEnvironment → Decide → ComposeMessage` chain.
  - Shape: a **guard, not a behavior change** (subagent + review, the R9b pattern — NOT tdd-commit, because its RED test passes immediately on already-correct code and would hit the `redConfirmed=false` gate). The value is mutation-sensitivity. All load-bearing pieces mutation-verified (each broke→failed→restored→green): advisory emission in message.go, `LimitedVisibility` derivation in decide.go:224, `PsiApplies` polarity in decide.go:281. Reviewer independently re-ran the advisory-emission break.
  - This is the Tier-1 `healthy/deadzone` recording guard F17's whole "output byte-identical" claim rests on.
