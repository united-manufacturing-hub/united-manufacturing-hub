...
func (s *TriggeringNextCycleState) Next(snapAny any) fsmv2.NextResult[any, any] {
	...
	if snap.Observed.ConnectionHealth == "healthy" {
		return fsmv2.Result[any, any](s, fsmv2.SignalNone, &action.DisconnectAction{}, "disconnecting to trigger next failure cycle", nil)
	}

	if snap.Observed.ConnectionHealth == "no connection" {
		return fsmv2.Result[any, any](&DisconnectedState{}, fsmv2.SignalNone, nil, "cycle triggered, connection lost", nil)
	}

	return fsmv2.Result[any, any](s, fsmv2.SignalNone, nil, "waiting for disconnect to complete", nil)
}
