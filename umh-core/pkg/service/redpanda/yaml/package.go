...
func RenderRedpandaYAML(input, output, pipeline, cacheResources, rateLimitResources, buffer interface{}, metricsPort int, logLevel string) (string, error) {
	// Create a config object from the individual components
	cfg := config.RedpandaServiceConfig{
...
func NormalizeRedpandaConfig(cfg config.RedpandaServiceConfig) config.RedpandaServiceConfig {
	return defaultNormalizer.NormalizeConfig(cfg)
}

// ConfigsEqual is a package-level function for easy config comparison
func ConfigsEqual(desired, observed config.RedpandaServiceConfig) bool {
...
func ConfigDiff(desired, observed config.RedpandaServiceConfig) string {
