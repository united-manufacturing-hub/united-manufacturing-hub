...
- GitHub Release notes are automatically populated from CHANGELOG.md by the `update-github-release` job in `build-umh-core.yml`. The job extracts the version's section, transforms headers for standalone display, and appends a changelog.umh.app link. You can still edit the Release body manually after if needed.
