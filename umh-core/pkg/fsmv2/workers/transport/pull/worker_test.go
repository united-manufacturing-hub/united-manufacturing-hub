...
var _ = Describe("PullWorker", func() {
	...
	BeforeEach(func() {
		...
		register.SetDeps[*transport.TransportDependencies]("transport", parentDeps)
		var err error
		pullDeps, err = pull.NewPullDependencies(parentDeps, identity, logger, nil)
		Expect(err).ToNot(HaveOccurred())
	})

	AfterEach(func() {
		register.ClearDeps("transport")
	...
	Describe("NewPullWorker", func() {
		It("should create a worker with valid args", func() {
			w, err := pull.NewPullWorker(identity, logger, nil, pullDeps)
			Expect(err).ToNot(HaveOccurred())
			Expect(w).NotTo(BeNil())
			var ok bool
			worker, ok = w.(*pull.PullWorker)
			Expect(ok).To(BeTrue())
	...
	Describe("DeriveDesiredState", func() {
		...
		It("should return running state for nil spec", func() {
			...
			wds := desired.(*fsmv2.WrappedDesiredState[pull.PullConfig])
			Expect(wds.Config.GetState()).To(Equal("running"))
		})
		...
		It("should return correct state for valid spec", func() {
			...
			Expect(desired).NotTo(BeNil())
			wds := desired.(*fsmv2.WrappedDesiredState[pull.PullConfig])
			Expect(wds.Config.GetState()).To(Equal("stopped"))
		})
		...
		It("should be deterministic", func() {
			...
			wds1 := desired1.(*fsmv2.WrappedDesiredState[pull.PullConfig])
			wds2 := desired2.(*fsmv2.WrappedDesiredState[pull.PullConfig])
			Expect(wds1.Config.GetState()).To(Equal(wds2.Config.GetState()))
		})
	...
	Describe("Factory registration", func() {
		...
		It("should read parent deps via register.SetDeps when instantiated via factory", func() {
			...
			pw, ok := w.(*pull.PullWorker)
			Expect(ok).To(BeTrue())
			Expect(pw).NotTo(BeNil())

			// The constructed worker must reflect the parent deps seeded via register.SetDeps
