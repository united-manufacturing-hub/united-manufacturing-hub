...
func NewAgentInstance(config config.AgentMonitorConfig) *AgentInstance {
	return NewAgentInstanceWithService(config, agent_monitor.NewAgentMonitorService(agent_monitor.WithFilesystemService(filesystem.NewDefaultService()), agent_monitor.WithS6Service(s6_orig.NewDefaultService())))
}
