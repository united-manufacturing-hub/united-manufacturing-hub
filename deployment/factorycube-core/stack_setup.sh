...
updateSystemPackages () {
  ...
  apt-get update && apt-get upgrade --yes > /dev/null

  if [[ $? -eq 0 ]]; then
    echo "System updates installed successfully"
    return 0
  else
    echo "Failed to update system packages. The installation will continue, although problems might occur while running the stack."
    return 1
  fi
}
...
installDocker() {
  ...
  checkInstallOlderVersion docker --version 2 $DOCKER_VERSION
  local check_result=$?

  if [[ $check_result -eq 0 ]]; then
    # Return 0 if the current version is equal or older than the required one
    printFoundAndCompatible "Docker"
    return 0
  elif [[ $check_result -eq 1 ]]; then
    # Uninstall the current version if it is older than the required one
    echo "Removing old docker versions. Found version: ${CURRENT_INSTALLED_VERSION}."
    apt-get remove docker docker-engine docker.io containerd runc > /dev/null
  fi

  # Install new version of docker with the convenience script
  echo "Installing Docker engine"
  curl -fsSL get.docker.com -o get-docker.sh
  sh get-docker.sh
  [[ $? -eq 0 ]] && echo "Docker engine installed successfully" || echo "Docker engine installation failed"
...
installDockerCompose() {
  ...
  apt-get install -y python3 python3-pip libffi-dev python3-dev libssl-dev
