...
func (m *BenthosManager) Reconcile(ctx context.Context, snapshot *public_fsm.SystemSnapshot, filesystemService filesystem.Service, tick uint64) (error, bool) {
	start := time.Now()
	defer func() {
	...
	err, reconciled := m.BaseFSMManager.Reconcile(ctx, &snapshotWithPorts, filesystemService, tick)
