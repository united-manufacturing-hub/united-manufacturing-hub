...
type IStreamProcessorService interface {
	...
	//  3. Converting the DFC config back to StreamProcessor format via
	...
	// reconcile equation:
	GetConfig(ctx context.Context, services serviceregistry.Provider, spName string) (streamprocessorserviceconfig.StreamProcessorServiceConfigRuntime, error)
	...
	AddToManager(ctx context.Context, services serviceregistry.Provider, cfg *dataflowcomponentserviceconfig.DataflowComponentServiceConfig, spName string) error

	// UpdateInManager updates an existing Stream Processor in the  DFC manager
	UpdateInManager(ctx context.Context, services serviceregistry.Provider, cfg *dataflowcomponentserviceconfig.DataflowComponentServiceConfig, spName string) error

	// RemoveFromManager removes a Stream Processor from the  DFC manager
	RemoveFromManager(ctx context.Context, services serviceregistry.Provider, spName string) error

	// Start starts a Stream Processor
	Start(ctx context.Context, services serviceregistry.Provider, spName string) error

	// Stop stops a Stream Processor
	Stop(ctx context.Context, services serviceregistry.Provider, spName string) error

	// ForceRemove removes a Stream Processor from the  DFC manager
	ForceRemove(ctx context.Context, services serviceregistry.Provider, spName string) error
	...
	ServiceExists(ctx context.Context, services serviceregistry.Provider, spName string) bool
...
// GetConfig pulls the **actual** runtime configuration that is currently
// deployed for the given Stream Processor. This is used for comparison
// with the desired DFC config generated by BuildRuntimeConfig.
func (p *Service) GetConfig(
	...
	dfcConfig, err := p.dataflowComponentService.GetConfig(ctx, services, underlyingDFCName)
	if err != nil {
...
func (p *Service) Status(
	...
	if !p.ServiceExists(ctx, services, spName) {
...
func (p *Service) AddToManager(
	...
	for _, config := range p.dataflowComponentConfig {
		if config.Name == underlyingDFCName {
...
func (p *Service) ServiceExists(
	...
	dfcExists := p.dataflowComponentService.ServiceExists(ctx, services, dfcName)
...
func (c *Service) ForceRemove(
	...
	err := c.dataflowComponentService.ForceRemoveDataFlowComponent(ctx, services, dfcName)
