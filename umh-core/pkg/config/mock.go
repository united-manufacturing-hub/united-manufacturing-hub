...
// getConfigUnsafe returns the config without acquiring any locks - should only be called when already holding a lock
func (m *MockConfigManager) getConfigUnsafe(ctx context.Context, tick uint64) (FullConfig, error) {
...
func (m *MockConfigManager) GetConfig(ctx context.Context, tick uint64) (FullConfig, error) {
	...
	return m.getConfigUnsafe(ctx, tick)
}
...
// IsGetConfigCalled returns the GetConfigCalled flag thread-safely
func (m *MockConfigManager) IsGetConfigCalled() bool {
	m.mutexReadOrWrite.Lock()
	defer m.mutexReadOrWrite.Unlock()
	return m.GetConfigCalled
}
