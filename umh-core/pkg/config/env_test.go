...
`
	...
	Context("when ENABLE_RESOURCE_LIMIT_BLOCKING environment variable has invalid value", func() {
...
`
		BeforeEach(func() {
			writtenData = []byte(validYAML)
			Expect(os.Setenv("ENABLE_RESOURCE_LIMIT_BLOCKING", "invalid")).To(Succeed())
		...
		It("should keep existing config value", func() {
			Eventually(func() error {
				wrappedManager := &FileConfigManagerWithBackoff{
					configManager: configManager,
				}
